...
def solve_model(buyers: List[Dict], products: List[Dict], seller_global_moq: int = 80) -> Tuple[Dict, float]:
    ...
    for buyer in buyers:
        ...
        z[buyer["name"]] = pulp.LpVariable(f"z_{buyer['name']}", lowBound=0, upBound=1, cat="Binary")
    ...
    for product in products:
        ...
        for buyer in buyers:
            n_mult = pulp.LpVariable(f"n_{buyer['name']}_{prod_id}", lowBound=0, cat="Integer")
    ...
    for buyer in buyers:
        ...
        buyer_total = 0
        for prod_id in buyer["products"]:
            alloc_value = x[(buyer["name"], prod_id)].value()
...
def run_auto_bid_aggressive(buyers: List[Dict], products: List[Dict], increment: float = 0.05, max_rounds: int = 30) -> List[Dict]:
    ...
    for round_num in range(max_rounds):
        ...
        for buyer in current_buyers:
            ...
            for prod_id, prod_conf in buyer["products"].items():
                ...
                if current_alloc < qty_desired and current_price < max_price:
...
def calculate_recommendations(buyers: List[Dict], products: List[Dict], new_buyer_name: str) -> Dict:
